...
The suite covers four groups. Fixtures compile in from `fixtures/bench/`, with small (~11 lines), medium (~97 lines), and large (~723 lines) variants for each of four languages: Python, TypeScript, Rust, and Go.

### `parse`

Measures `get_parser(...).parse(...)` across all four languages at all three sizes. This is the baseline — a single tree-sitter parse with no post-processing.
