...
// alef:hash:692787a5727abdd7e79553839a58d9f06d0b2e20c4b341b6d9a3681f29a46da8
...
// Point Point source position — row + column, zero-indexed.
...
// ByteRange ByteRange byte range — start (inclusive) to end (exclusive).
...
// GetParser GetParser get a [`Parser`] pre-configured for the given language.
...
// SetLanguage SetLanguage configure the parser to use the language identified by name (e.g. `"python"`).
...
// Parse Parse a UTF-8 source string. Returns `None` if parsing was cancelled
...
// ParseBytes ParseBytes parse a raw byte slice. Returns `None` if parsing was cancelled or

// no language is set.
func (h *Parser) ParseBytes(
	source []byte) *Tree {
...
// Reset Reset internal state. The next call to [`parse`](Self::parse) will
...
// RootNode RootNode return the root [`Node`] of this tree.
...
// Walk Walk return a [`TreeCursor`] positioned at the root.
...
// Kind Kind return the node's kind name (e.g. `"function_definition"`).
...
// KindID KindID return the node's numeric kind ID.
...
// StartByte StartByte return the inclusive start byte offset of this node.
...
// EndByte EndByte return the exclusive end byte offset of this node.
...
// ByteRange ByteRange return the node's byte range as a [`ByteRange`].
...
// StartPosition StartPosition return the start [`Point`] (row, column).
...
// EndPosition EndPosition return the end [`Point`] (row, column).
...
// IsNamed IsNamed true when this node is named (not punctuation/whitespace).
...
// IsError IsError true when this is an error node.
...
// IsMissing IsMissing true when this is a missing-token node.
...
// IsExtra IsExtra true when this is an "extra" node (e.g. a comment).
...
// HasError HasError true when this node or any descendant is an error.
...
// Parent Parent return this node's parent, if any.
...
// Child Child return the i-th child of this node, if any.
...
// ChildCount ChildCount total number of children (including unnamed).
...
// NamedChild NamedChild return the i-th named child of this node, if any.

func (h *Node) NamedChild(
	index uint32) *Node {
	cIndex := C.uint32_t(uint32(index))
...
// NamedChildCount NamedChildCount number of named children of this node.
...
// ChildByFieldName ChildByFieldName look up a child by its grammar-defined field name.

func (h *Node) ChildByFieldName(
	name string) *Node {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
...
// ToSexp ToSexp return the S-expression form of this node's subtree.
...
// Walk Walk return a [`TreeCursor`] positioned at this node.
...
// Node Node return the [`Node`] at the cursor's current position.
...
// GotoFirstChild GotoFirstChild move the cursor to the first child of the current node.
...
// GotoParent GotoParent move the cursor to the parent of the current node.
...
// GotoNextSibling GotoNextSibling move the cursor to the next sibling of the current node.
...
// FieldName FieldName return the field name for the current node, if any.
