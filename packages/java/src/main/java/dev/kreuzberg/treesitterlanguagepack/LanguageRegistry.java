 ...
 *
 * Manages both statically compiled and dynamically loaded language grammars. Use LanguageRegistry.new() for the default registry, or access
 * the global instance via the module-level convenience functions ({@code get_language}, {@code available_languages}, etc.).
 */
...
public class LanguageRegistry implements AutoCloseable {
     ...
     *
     * Resolves aliases (e.g., {@code "shell"} -&gt; {@code "bash"}, {@code "makefile"} -&gt; {@code "make"}), then looks up the language in
     * the static table. When the {@code dynamic-loading} feature is enabled, falls back to loading a shared library on demand.
     * {@literal @}throws Exception Returns Error.LanguageNotFound if the name (after alias resolution) does not match any known grammar.
     */
     ...
     *
     * Includes statically compiled languages, dynamically loadable languages (if the {@code dynamic-loading} feature is enabled), and all
     * configured aliases.
     */
     ...
     *
     * Returns {@code true} only when the grammar was compiled in at build time (i.e. it appears in the {@code STATIC_LANGUAGES} table).
     * This is independent of the extension-to-language mapping: {@code detect_language_from_extension} consults the static ext table for
     * all 306 grammars regardless of which parsers are compiled in.
     *
     ...
     *
     * let registry = LanguageRegistry.new(); // Extension detection uses the static table — independent of compiled parsers. let lang =
     * detect_language_from_extension("feature"); // always returns the value ("gherkin") // Parser availability depends on which grammars
     * were compiled in. let can_parse = lang.map(|name| registry.has_parser(name)).unwrap_or(false);
     *
     */
     ...
     *
     * Returns {@code true} if the language can be loaded, either from the static table or from a dynamic library on disk.
     */
    public boolean hasLanguage(final String name) throws TreeSitterLanguagePackRsException {
        ...
        try {
            Arena arena = Arena.ofAuto();
            var cName = arena.allocateFrom(name);
            var result = (int) NativeLib.TS_PACK_LANGUAGE_REGISTRY_HAS_LANGUAGE.invoke(this.handle, cName);
            return result != 0;
        } catch (TreeSitterLanguagePackRsException ex) {
            throw ex;
        } catch (Throwable e) {
     ...
     *
     * When the {@code dynamic-loading} feature is enabled, the registry also knows about dynamically loadable grammars and will load them
     * on demand.
     */
