...
public class DownloadManager implements AutoCloseable {
     ...
     *
     * Unlike Self.ensure_languages, this does not check the manifest language list against archive contents — it simply extracts all
     * {@code .so}/{@code .dylib}/{@code .dll} files from the bundle. Languages in the manifest that are missing from the archive are
     ...
     *
     * Acquires the cross-process lock so {@code clean_cache} cannot race a concurrent downloader (avoids Windows sharing-violation errors
     * against an in-flight bundle write). The {@code .download.lock} file itself is **not** removed — it is permanent infrastructure;
     * deleting it could allow a concurrent process that already opened the file to continue holding a stale lock handle while a new process
     * opens a fresh inode, breaking the mutual-exclusion guarantee.
     */
