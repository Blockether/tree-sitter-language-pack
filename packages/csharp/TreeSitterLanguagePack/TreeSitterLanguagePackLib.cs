...
// alef:hash:3674dcf8c55882c8115d60e9f2500380ccf46ee4b06cc15024bd147613900cbc
#nullable enable
...
public static class TreeSitterLanguagePackLib
{
    ...
    public static ProcessResult Process(string source, ProcessConfig? config)
    {
        ...
        var returnValue = new ProcessResult(nativeResult);
    ...
    {
        var configJson = JsonSerializer.Serialize((config ?? new PackConfig()), JsonOptions);
        var configHandle = NativeMethods.PackConfigFromJson(configJson);
        if (configHandle == IntPtr.Zero)
        {
            var ec = NativeMethods.LastErrorCode();
            var ctxPtr = NativeMethods.LastErrorContext();
            var msg = Marshal.PtrToStringUTF8(ctxPtr) ?? "PackConfigFromJson failed";
            throw new TreeSitterLanguagePackException(ec, msg);
        }
        NativeMethods.Init(
            configHandle
    ...
    public static PackConfig PackConfigFromTomlFile(string path)
    {
        ...
        var returnValue = new PackConfig(nativeResult);
        return returnValue;
    }
    ...
    public static Point PointFrom(Point p)
    {
        ArgumentNullException.ThrowIfNull(p);
        var pJson = JsonSerializer.Serialize(p, JsonOptions);
        var pHandle = NativeMethods.PointFromJson(pJson);
        if (pHandle == IntPtr.Zero)
        {
            var ec = NativeMethods.LastErrorCode();
            var ctxPtr = NativeMethods.LastErrorContext();
            var msg = Marshal.PtrToStringUTF8(ctxPtr) ?? "PointFromJson failed";
            throw new TreeSitterLanguagePackException(ec, msg);
        }
        var nativeResult = NativeMethods.PointFrom(
            pHandle
        );
        var returnValue = new Point(nativeResult);
        NativeMethods.PointFree(pHandle);
        return returnValue;
    }
    ...
    public static ProcessConfig ProcessConfigDefault()
    {
        ...
        var returnValue = new ProcessConfig(nativeResult);
        return returnValue;
    }
    ...
    private static Exception GetLastError()
    {
        ...
        var ctxPtr = NativeMethods.LastErrorContext();
        var message = Marshal.PtrToStringUTF8(ctxPtr) ?? "Unknown error";        if (message.StartsWith("Language function returned null pointer for '")) return new NullLanguagePointerException(message);        if (message.StartsWith("Parse failed: parsing returned no tree")) return new ParseFailedException(message);        if (message.StartsWith("Failed to set parser language:")) return new ParserSetupException(message);        if (message.StartsWith("Dynamic library load error:")) return new DynamicLoadException(message);        if (message.StartsWith("Registry lock poisoned:")) return new LockPoisonedException(message);        if (message.StartsWith("Configuration error:")) return new ConfigException(message);        if (message.StartsWith("Invalid byte range:")) return new InvalidRangeException(message);        if (message.StartsWith("Query error:")) return new QueryErrorException(message);        if (message.StartsWith("Language '")) return new LanguageNotFoundException(message);        if (message.StartsWith("IO error:")) return new IoException(message);        if (code == 2) return new ErrorException(message);        return new TreeSitterLanguagePackException(code, message);
    }
}
